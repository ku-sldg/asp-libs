...
struct ASP_ARGS_Run_Command_Appr {
...
fn body(ev: copland::ASP_RawEv, args: copland::ASP_ARGS) -> Result<Result<()>> {
    ...
    let myaspargs: ASP_ARGS_Run_Command_Appr = serde_json::from_value(args)
    .context("Could not parse ASP_ARGS for ASP run_command_verus_appr")?;

    let targid = myaspargs.targ_id_appr.clone();

    if targid == "run_coq_theorem_test_targ".to_string() || targid == "run_coq_theorem_targ".to_string()
        {
        ...
        if targid == "run_verus_targ".to_string() {
        ...
        else {Ok(Err(anyhow::anyhow!("Appraisal was not successful")))}
    }
}
//}
