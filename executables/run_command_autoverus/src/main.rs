...
struct ASP_ARGS_RunCommandAutoVerus {
    exe_args: Vec<String>
}
...
fn body(_ev: copland::ASP_RawEv, args: copland::ASP_ARGS) -> Result<copland::ASP_RawEv> {
    ...
    let myaspargs : ASP_ARGS_RunCommandAutoVerus = serde_json::from_value(args)
    .context("Could not decode JSON ASP_ARGS for ASP run_command_autoverus")?;

    let command_string = "python3".to_string(); //"verus".to_string();
